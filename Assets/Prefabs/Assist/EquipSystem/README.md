...
2. **소켓 만들기**: 캐릭터 프리팹을 열고 `Tools → EquipSystem → Socket Maker`
   ...
   - **휠=거리 띄우기, Ctrl+휠=ZX 회전, Shift+휠=YZ 회전, Ctrl+Shift+휠=XY 회전, R=리셋, Esc=취소**
     — 축은 클릭 표면 기준(오른쪽=X, 법선=Y, 접선=Z). 회전 평면 3개(ZX/YZ/XY) 완비라
     어떤 방향으로든 돌릴 수 있다 (ZX=표면 위에서 빙글, YZ=앞뒤 기울임, XY=좌우 눕히기)
   - **클릭 = 후보 고정(검수)** — 고스트가 그 자리에 멈추고 아직 소켓은 안 생긴다.
     검수 중에는 카메라가 자유(맨휠 줌·우클릭 궤도·중클릭 팬 그대로) + **[턴테이블]**·5방향
     시점 버튼(정면/후면/좌/우/상)으로 여러 각도 확인 가능.
     **표면 지점만 고정될 뿐 조정은 계속 가능** — 회전(Ctrl/Shift/Ctrl+Shift+휠 그대로),
     **거리(Alt+휠 또는 [거리 ±5%] 버튼)**, **크기([크기 ±0.1] 버튼 = 카탈로그 sizeRatio)**,
     **R=전부 초기화**(회전 3축+거리+세션 중 바꾼 크기)
...
3. **이름 짓기**: 생성된 소켓 인스펙터에서 slotId를 의미 있는 이름(hairpin 등)으로 —
   **GO명과 카탈로그가 자동 동기화**된다.
   - 아이템이 이미 의미 있는 자리(head1 등)에 등록돼 있으면 **승인 시 3지선다 다이얼로그**:
     그 자리 소켓이 캐릭터에 **있으면** ① 그 소켓 덮어쓰기 ② 아이템 key 이름으로 새 소켓 ③ 임시 이름 /
     **없으면** ① 이 소켓을 그 이름으로 ② key 이름으로 ③ 임시 이름. (버튼 = "1/2/3 수행", Esc=3)
   - **카탈로그 등록(targetSlotId)은 어떤 선택에서도 바뀌지 않는다** — 전역 장부는 읽기 전용,
     자동 쓰기는 "첫 등록"(빈 값·임시 socket_* 값)일 때만.
