...
- **지갑 API 의미론 분리** (당시 `InventoryManager` — 이후 골드 단일화로 `CurrencyManager`의
  `Earn/Add/Refund/Spend`로 계승됨):
  `Earn`=소득(잔액+누적 획득 → CH0001 반응) · `Add`=순수 잔액 변경(누적 집계 무반응, db성 변경) ·
  `Refund`=실패 결제 되돌림(잔액 복구+누적 소비 차감 — **CH0007 진행 후퇴는 의도된 동작**) ·
  `Spend`=소비(누적 소비 가산 → CH0007 반응).
