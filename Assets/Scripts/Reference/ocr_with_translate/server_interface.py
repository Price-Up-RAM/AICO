...
def get_furigana():
    ...
    def generate():
        ...
        # 2. AI 호출로 후리가나 변환
        try:
            furigana_result = ai_furigana.process(text)
...
def main_stream():  # main logic
    ...
    # VL 모델 사용 여부 최종 결정: 이미지가 있고 + 멀티모달 모델인 경우
    if image_info and has_multimodal:
        is_custom_vl_model = True
        print(f'### VL model will be used: {using_model_name} with image')
    ...
    def generate(image_info):
        ...
        if server_type == "OpenRouter":  # OpenRouter API 사용
            ...
            if (query_intent != "image" or True) and (intent_image == 'on' and not is_intent_image):
                ...
                web_keyword = ai_web_search_keyword.process(query, image_info, lang=lang_infer_type)
                _, web_keyword = web_keyword.split("keyword:", 1)
...
def main_stream_multi():
    from server_multi_impl import main_stream_multi, set_translator
    # server_temp에 현재 translator 주입
    try:
...
if __name__ == '__main__': 
    ...
    if not state.model_name:
        ...
        server_config["model_type"] = 'Qwen3-8B-Q4_K_M.gguf'
