...
def set_default_response_format_intent_info():
    ...
    intent_info['web_search_method'] = ''          # 웹 검색 수단 (duckduckgo, Tavily 등)
    intent_info['web_search_content'] = ''         # 웹 검색 원본 내용
...
def check_individual_intent_image(query, image_info, intent_image, lang_infer_type, image_info_text=None):
    intent_image = intent_image.lower()
    if intent_image == 'force':
        return True
    if intent_image == 'off':
        return False
