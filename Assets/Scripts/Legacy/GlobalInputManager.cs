...
// LEGACY - 더 이상 사용하지 않음
// ==================================================================================
// 이 클래스는 레거시 코드로, 다음의 3개 매니저로 분리되었습니다:
// - GlobalInputKeyboardManager: 키보드 입력 전역 후킹
// - GlobalInputMouseClickManager: 마우스 클릭 전역 후킹
// - GlobalInputMouseMoveManager: 마우스 이동 전역 후킹 (성능 이슈로 기본 비활성)
//
// 이 파일은 참고용으로만 유지되며, 추후 제거 예정입니다.
// ==================================================================================
    ...
    private static IntPtr KeyboardHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0 && _instance != null && _instance.inputDetectionEnabled)
        {
            if (wParam == (IntPtr)WM_KEYDOWN || wParam == (IntPtr)WM_SYSKEYDOWN)
            {
                ...
                if (vkCode == 0x77) // F8 (레거시 코드 - GlobalInputKeyboardManager 사용 권장)
                {
                    _instance.PressF12();
                }
