# Affinity(인연도) 시스템 전환 계획

> 상태: **계획/브레인스토밍 단계** — 코드 수정 없음. 이 문서가 확정되면 구현 착수.

> **개정 이력**
> - 2026-07-11 — 구 `Relationship_Plan.md` 에서 리네임. 사유: "relationship"은 향후 다른 시스템의 예약어가 되어, 인연도의 코드 용어를 **affinity** 로 확정. 상점/재화 파트(구 5장)는 `CharacterDetail/Affinity_Store_Integration.md` 로 책임 이관.
...
- 기존 **호감도(affection, 0~300, 3단 바)** 시스템을 **인연도(affinity)** 시스템으로 전면 교체한다.
...
- **AI 행동 변화(호칭/말투/프롬프트 주입)는 보상에서 배제.** 구현 난도가 높아 호감도→인연도 전환의 이유이기도 함. 보상은 눈에 보이는 치장/해금 계열로 한정.
...
- 등록 대상: 단계 명칭 6종 + "인연도" 라벨 자체 (jp: 絆レベル 또는 親密度, en: Affinity) + 커스텀 입력 UI 문구(placeholder, 확인/초기화 버튼).
...
- **카드 테두리**: CharacterDetail 카드에 레벨 도달 시 테두리 표시 — **Lv4 동테 / Lv7 은테 / Lv10 금테**. 상점 판매 없는 인연도 전용 상징. (배지·전용 연출·의상 해금은 구현 난도로 폐기)
...
- Mission AF0004 "인연도 레벨업" 리포트 연결.
- 테두리 구현 메모: `CharacterDetail.prefab` 카드(또는 초상화) 외곽에 테두리 Image 1개를 두고 동/은/금 스프라이트(또는 색상) 스왑, Lv4 미만은 비표시.

## 5. 상점/재화 — 책임 이관

상점·재화·선물(증정→인연도 포인트) 설계는 이 문서에서 분리되어 **같은 폴더의 `Affinity_Store_Integration.md` 가 소유**한다. Store 프로토타입(`Assets/Prefabs/UI/Store`)이 이미 재화(Gold) 획득·소비 루프를 검증한 상태이므로, 재화 구조·선물 아이템 정의·상점 스코프 등의 미결 사항은 해당 문서에서 결정한다. 이 문서는 인연도의 레벨 모델·명칭·레벨업 보상까지만 다룬다.
...
| `CharacterDetailController.cs` | `SetAffection()` — "호감도 X/300" 텍스트 + 3단 바(Yellow/Orange/Red) | `SetAffinity()` — "인연도 Lv.N" + 단계 명칭 + 현 레벨 진행도 게이지 |
...
| `CharacterDetail.prefab` | `FeatureTag_호감도보유_Text` ("호감도 보유") | "인연도 보유" 또는 태그 제거 |
| `Scripts/LanguageData.cs` | — | 단계 명칭 6종 + 인연도 라벨 + 커스텀 UI 문구 등록 (`ko`/`jp`/`en`) |
...
6. 캐릭터별 독립 인연도(현행 char_code 단위) 유지 여부 — 복장이 다르면 인연도가 분리되는 현행 키 구조가 의도인지 확인 필요.
