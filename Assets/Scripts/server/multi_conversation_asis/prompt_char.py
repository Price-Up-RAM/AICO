...
def arona_info_content(lang='en'):  # 2261
...
def arona_situation_00_content(lang='en'):
    content = """### Situation
- Residence : a desolate classroom located in the midst of a watery expanse, stretching further than the eye can see. 
"""
    if lang == 'ko':
        content = """### 상황
- 거주지 : 눈에 보이는 끝보다도 멀리 뻗어 있는 물로 가득 찬 광활한 공간 한가운데에 위치한 황량한 교실.
"""
    elif lang == 'ja' or lang == 'jp':
        content = """### 状況
- 住居 : 見渡す限り水に囲まれた広大な空間の中央にぽつんと佇む荒れ果てた教室。
"""
    
    return content

def get_char_info_from_json(file_name, lang='en'):
    file_path = './prompt/' + file_name + '.json'
    # 한국어/일본어 프롬프트 시도
    if lang == 'ja' or lang == 'jp':
        file_path = './prompt/ja/'+ file_name + '.json'
        if not os.path.exists(file_path):
            file_path = './prompt/' + file_name + '.json'
    if lang == 'ko':
        file_path = './prompt/ko/'+ file_name + '.json'
        if not os.path.exists(file_path):
            file_path = './prompt/' + file_name + '.json'
