...
def patch(path, old, new, label):
    ...
    n = s.count(old)
    if n != 1:
        print("FAIL %s : 앵커 %d회 매치 (1이어야 함)" % (label, n))
        return False
...
ok &= patch("SettingCharManager.cs",
"""        isLoaded = true;
...
"""        if (string.IsNullOrEmpty(charCode)) return null;
        ...
        if (!settingsCharData.char_code_info_dict.ContainsKey(charCode))
        {
            ...
            Debug.Log($"[SettingChar/miss] charcode='{charCode}' 엔트리 없어 빈 값 생성 → voiceId='' (ref_id 누락) | 제안: LoadSettingChar 시딩 조건을 '파일없음'에서 '엔트리없음'으로 확장하면 woman_01");
            // 조회만으로는 저장하지 않는다 — 실제 변경(AddAffinityPoints/SetVoice 등)이 SaveToFile을 수행
            settingsCharData.char_code_info_dict[charCode] = new CharCodeSetting();
        }""",
...
"""        GameObject currentCharacter = CharManager.Instance != null ? CharManager.Instance.GetCurrentCharacter() : null;
        ...
        {
            Debug.Log($"[TTS/refid] charcode='{diagCharcode}' nickname='{nickname}' | voiceId='' → ref_id 누락 | 직전 [SettingChar/*] 로그에서 원인 확인");
        }
        else
        {
