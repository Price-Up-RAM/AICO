...
public class ChatBalloonManager : MonoBehaviour
{
    ...
    // =========================================================
    // 접근성 모드 예외 — MR 손 프레임 사진 (2026-08-26)
    // =========================================================
    // KAIManager.IsAccessibilityModeActive는 '씬에 KAIManager가 있으면 true'다.
    // MR 씬에는 캐릭터 교체·MenuTriggerKAI 때문에 KAIManager가 상주하므로
    // 접근성 모드가 항상 켜진 상태가 되고, 이미지 게이트가 전부 닫혀 있었다.
    // 실측(Editor.log): OnMRScreenshotCaptured가 force로 올려도
    // GetUseImageInfo()가 무조건 "off"를 돌려줘 intent_image=off로 나갔다.
    //
    // 데스크톱 접근성 동작은 그대로 두고, '사용자가 직접 손으로 찍어 주입한 사진'이
    // 실제로 있을 때만 예외로 통과시킨다. 클립보드·데스크톱 영역 캡처는 계속 막힌다.
    private bool HasMRPhoto()
    {
        ...
        return sm.HasMRScreenshot;
    }

    // 이미지 기능이 잠겨 있는가. MR 사진이 있으면 잠기지 않는다.
    private bool IsImageFeatureLocked()
    {
    ...
    // 단 MR 손 프레임 사진이 주입돼 있으면 첨부를 해제할 수 있어야 하므로 버튼을 살린다.
    public void ApplyImageFeatureAvailability()
    {
        bool isAvailable = !IsImageFeatureLocked();
    ...
    // =========================================================
    // 손 프레임 스샷이 찍혔을 때 (MRHandFrameGesture가 부른다)
    // =========================================================
    // 사진을 ScreenshotManager에 주입하는 것만으로는 부족하다.
    //  - 사용자가 사진이 붙었는지 알 수 없다
    //  - 말풍선의 이미지 모드가 off면 ApiVlRouterManager.ApplyMRImageGuard가 다시 off로 강등한다
    // 그래서 말풍선을 열고 force로 올린 뒤 아이콘까지 갱신한다.
    public void OnMRScreenshotCaptured(int width, int height, int byteLength)
    {
        bool wasActive = IsChatBalloonActive();

        if (!wasActive)
        {
            // 닫혀 있으면 연다. 열려 있으면 지금 대화를 끊지 않고 그대로 붙인다.
            ShowChatBalloon();
        }

        // 손으로 직접 사각형을 만들어 찍은 것이라 의도가 명확하다.
        // 이전에 수동으로 off를 눌러 둔 상태여도 푼다.
        isManualOff = false;
        // 방금 찍은 사진이 클립보드보다 우선이다.
        ignoreClipboard = true;
        lastImageSource = "screenshot";
        SetUseImageInfo("force");

        // 접근성 모드 때문에 Awake에서 꺼져 있던 버튼을 되살린다.
        // 이제 MR 사진이 있으므로 IsImageFeatureLocked()가 false다.
        ApplyImageFeatureAvailability();

        bool nowActive = IsChatBalloonActive();

        // '세팅했다'와 '읽으면 그 값이 나온다'는 다른 사실이다 (Kickoff Guide 4-58).
        // 예전 로그는 필드값을 찍어서, 게터가 접근성 모드로 off를 돌려주는 것을 놓쳤다.
        string effective = GetUseImageInfo();
        string source = GetImageSource();
        Debug.Log($"[ChatBalloon/MR첨부] 손 프레임 스샷 첨부 | {width}x{height} ({byteLength}바이트) | " +
                  $"말풍선 현재값={wasActive} → 새값={nowActive} | " +
                  $"useImageInfo 필드={useImageInfo} 게터={effective} | 소스={source} | " +
                  $"접근성모드={KAIManager.IsAccessibilityModeActive} MR사진={HasMRPhoto()}");
    }

    // 현재 이미지 소스의 이미지 미리보기
    private void ShowCurrentImage()
    {
        if (IsImageFeatureLocked())
        {
            return;
        }
        ...
        // MR에서 주입된 손 프레임 사진이 있으면 그것이 실제로 전송될 이미지다.
        // ShowScreenshotImage는 persistentDataPath의 panel_capture.png를 읽으므로
        // MR 경로에서는 엉뚱한 파일을 보여주거나 '파일 없음'이 된다.
        if (sm.HasMRScreenshot && sm.ShowMRScreenshotImage())
        {
            return;
        }
...
// #endif
    ...
    //
    // 주의: 말풍선을 닫았다 다시 열면 PrepareChatBalloon이 이걸 부른다.
    // MR 사진이 붙어 있는 채로 여기서 off/isManualOff로 되돌리면
    // 사진은 남아 있는데 전송은 안 되는 상태가 된다. 그래서 예외를 함께 본다.
    public void InitUseImageInfo()
    {
        ...
        // MR 손 프레임 사진이 붙어 있으면 설정값(ai_use_image_idx)으로 덮지 않는다.
        // 사용자가 방금 직접 찍은 것이 설정 기본값보다 우선한다.
        if (HasMRPhoto())
        {
            isManualOff = false;
            ignoreClipboard = true;
            lastImageSource = "screenshot";
            SetUseImageInfo("force");
            Debug.Log("[ChatBalloonManager] MR 사진이 붙어 있어 설정값 대신 force를 유지한다");
            return;
        }
    ...
    public void changeUseImageInfo()
    {
        ...
        else if (useImageInfo == "force")
        {
            ...
            // MR 손 프레임 사진은 이 시점이 '명시적으로 지우는' 순간이다.
            // 여기서 지우지 않으면 다음 질문에도 계속 따라붙는다.
            ScreenshotManager smOff = FindObjectOfType<ScreenshotManager>();
            if (smOff != null && smOff.HasMRScreenshot)
            {
                smOff.ClearMRScreenshot();
            }
        }
