...
### 2026-07 개정: 타임아웃 앵커 = "재생 유휴 진입" 기준

* 기존 구현은 시작 시각을 "이전 seq 큐 삽입 시점"에 기록해, 앞 문장 재생이 2초보다
  길기만 해도 뒷문장이 연쇄 skipped 되는 결함이 있었다 (긴 답변 뒷부분 무음).
* 개정: 재생 파이프라인(메인 재생/큐/로드 + 서브 발화)이 busy인 동안은 시작 시각을
  계속 뒤로 밀고, 유휴 진입 후부터 2초를 센다. seq=0 무한 대기는 유지.
* 추가: skipped 처리된 seq의 wav가 뒤늦게 도착했을 때, 그 뒤 문장이 아직 하나도
  재생되지 않았다면 nextSeqToPlay를 되돌려 복구한다 (MarkTtsReady).
