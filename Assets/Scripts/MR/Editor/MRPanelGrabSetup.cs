...
namespace AICO.MR.EditorTools
{
    public static class MRPanelGrabSetup
    {
        ...
        // 콜라이더의 앞뒤 두께(m).
        //
        // 너무 얇으면(8mm 시절) **핀치 grab이 거의 안 걸린다** — 핀치 지점(엄지-검지 중간)이
        // 얇은 판 안에 들어가야 하는데 확률이 너무 낮다. 손바닥 grab은 판정 여유가 커서
        // 되지만 핀치만 안 되는 증상이 나온다 (실기 확인 2026-08-15).
        //
        // 대신 두께를 늘려도 **뒤쪽으로만** 늘어나게 한다 — MRGrabFrameFitter가 Z를
        // `깊이/2 + 여유`만큼 미므로 앞면 위치는 그대로다. 즉 콜라이더가 UI 앞으로
        // 튀어나오지 않아 레이 조준 우선권은 유지된다.
        private const float BandDepth = 0.04f;
