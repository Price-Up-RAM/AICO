...
{
    [MenuItem("Tools/MR/강제로 arona655 서버 설정하기")]
    public static void ForceSet()
    {
        ...
        if (File.Exists(file))
        {
            ...
            // 아주 단순무식하게 텍스트 교체 (temp -> arona655, server_type_idx 강제 10)
            json = System.Text.RegularExpressions.Regex.Replace(json, "\"server_id\":\\s*\".*?\"", "\"server_id\": \"arona655\"");
            json = System.Text.RegularExpressions.Regex.Replace(json, "\"server_type_idx\":\\s*\\d+", "\"server_type_idx\": 10");
            File.WriteAllText(file, json);
            Debug.Log("[MR] settings.json 파일을 arona655 (Type 10)로 강제 덮어씌웠습니다! 이제 플레이 해보세요.");
        }
        else
        {
